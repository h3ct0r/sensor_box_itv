...
Mat Image::computeEdgeImage()
{
  ...
  //cv::threshold(grayImg, grayImg, 30, 255, cv::THRESH_BINARY);
...
bool Image::detect4Circles(float canny_thresh, float center_thresh, vector<Point2f> &centers, vector<float> &radiuses)
{
  ...
  // namedWindow("Gray tresh", CV_WINDOW_AUTOSIZE);
  // imshow("Gray thresh", src_gray);
  // waitKey(0);

  for (int thresh = center_thresh; circles.size() < 4 && thresh > 10; thresh -= 5)
  {
    HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows / 32, canny_thresh, thresh, 0, 0);
  }
